...
TC07: ตรวจสอบ UI ครบถ้วน
    [Documentation]    ตรวจสอบว่าหน้าลบบัญชีแสดง element ครบทุกส่วน
    [Tags]    delete_account    ui
    ...
    # หัวข้อและคำเตือน
    Page Should Contain    การลบบัญชีผู้ใช้
    Page Should Contain    หากลบบัญชี

    # Section เลือกข้อมูล
    Page Should Contain    ข้อมูลส่วนบุคคลที่ต้องการจะรับ
    Page Should Contain Element    xpath=//label[contains(.,'ประวัติส่วนตัว')]//input[@type='checkbox']
    Page Should Contain Element    xpath=//label[contains(.,'ประวัติการเดินทาง')]//input[@type='checkbox']
    Page Should Contain Element    xpath=//label[contains(.,'ประวัติการสร้างเส้นทาง')]//input[@type='checkbox']
    Page Should Contain    กรุณาเลือกข้อมูลอย่างน้อย 1 รายการ

    # Checkbox รับทราบ
    Page Should Contain Element    xpath=//label[contains(.,'รับทราบข้อตกลง')]//input[@type='checkbox']

    # Input ยืนยัน
    Page Should Contain Element    css=input[placeholder="พิมพ์คำว่า ยืนยัน"]

    # ปุ่มลบ (ต้อง disabled ตอนแรก)
    Page Should Contain Element    xpath=//button[contains(text(),'ยืนยันการลบข้อมูล')]
    ตรวจสอบว่าปุ่มยืนยันลบยัง Disabled อยู่
