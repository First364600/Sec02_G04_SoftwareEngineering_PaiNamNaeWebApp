...
TC-3.3 & 3.4: Tamper Detection (HMAC Catching DB Modification)
    [Documentation]    จำลองการแฮกแก้ข้อมูลใน DB และเช็คว่า API ตรวจจับได้หรือไม่
    # 1. จำลองการแก้ไข Database โดยตรง (เปลี่ยน statusCode เป็น 500)
    Execute Sql String    UPDATE "SystemLog" SET "statusCode" = 500 WHERE id = '${LATEST_LOG_ID}';
    
    # 2. ยิง API ตรวจสอบอีกครั้ง
    ${headers}=    Create Dictionary    Authorization=Bearer ${ADMIN_TOKEN}
    ${response}=    GET On Session    api    /api/users/admin/logs/${LATEST_LOG_ID}/verify    headers=${headers}
    Status Should Be    200    ${response}
    
    # 3. ต้องตรวจเจอว่าโดนแฮก (isValid ต้องเป็น False)
...
TC-3.5: Verify All Logs in First Page
    ...
    FOR    ${log}    IN    @{logs}
        ...
        # แก้ไขบรรทัดนี้: เติม url= และ headers= ให้ชัดเจน
