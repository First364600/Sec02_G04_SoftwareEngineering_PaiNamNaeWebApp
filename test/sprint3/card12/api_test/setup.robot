...
Setup All Sessions
    ...
    # Generate departure time from real current time (UTC) + 1 hour
    ${now_utc}=    Get Current Date    UTC
    ${departure_time}=    Add Time To Date    ${now_utc}    1 hour    result_format=%Y-%m-%dT%H:%M:%S.000Z
    Set Global Variable    ${ROUTE_DEPARTURE_TIME}    ${departure_time}
    Log To Console    ROUTE_DEPARTURE_TIME: ${ROUTE_DEPARTURE_TIME}
    ...
    # Driver Confirm user bookings 1
    ...
    # Driver Confirm user bookings 2
    Log To Console    Driver comfirm bookings
    ${response}=    Driver Confirm User Bookings
    ...    sessionName=DriverSession
    ...    bookingId=${BOOKING_ID}
    ...
    # สำหรับกรณีที่ ส่งข้อความทั้งที่การเดินทางยังไม่เริ่ม
    # driver create route 2
    ${response}=    Driver Create Route
    ...    sessionName=DriverSession
    ...    vehicleId=${ROUTE_VEHICLE_ID}
    ...    startLocation=${ROUTE_START_LOCATION}
    ...
    Set To Dictionary    ${ROUTES_LIST}    route2    ${response}[data]
    ${ROUTE_ID}=    Get From Dictionary    ${response}[data]    id
    # Passenger 1 Bookings Route 2 no Start
    Log To Console    Passenger 1 Bookings Route2
    ${response}=    Passenger Bookings Route
    ...    sessionName=Passenger1Session
    ...    routeId=${ROUTE_ID}
    ...    numberOfSeats=${P1_BOOKINGS_NUMBER_OF_SEATS}
    ...
    Set To Dictionary    ${BOOKINGS_LIST}    route2_book1    ${response}[data] 
